...
namespace GeoGen.AnalyticalGeometry.Test.AnalyticalObjects
{
    ...
    public class LineTest
    {
        ...
        public void Test_Construction_From_Equal_Points()
        {
            ...
            Assert.Throws<ArgumentException>(() => new Line(p1, p2));
            Assert.Throws<ArgumentException>(() => new Line(p1, p1));
            Assert.Throws<ArgumentException>(() => new Line(p2, p2));
        }
        ...
        public void Test_Line_Intersections()
        {
            ...
            Assert.Throws<ArgumentException>(() => ab.IntersectionWith(ab));
            Assert.Throws<ArgumentException>(() => ab.IntersectionWith(new Line(a, d)));
        ...
        public void Test_Perpendicular_Line()
        {
            ...
            foreach (var i in Enumerable.Range(0, points.Count))
            {
                ...
                Assert.IsTrue(normalized[0] == result.A);
                Assert.IsTrue(normalized[1] == result.B);
                Assert.IsTrue(normalized[2] == result.C);
            }
        }
    }
}